...
- Active repository pack: `tb26-final-supplemental-catalog-20260829132049`
...
- The private In-Season GM is live at `/thunder-bowl/season/`. Its single **Update everything** action uses the least-privilege Chrome Data Helper v0.4.0 to read CBS's authenticated all-team report and current-week component-stat projection tables, then refreshes Footballguys component-stat projections plus injury, depth, news, and IR evidence. Manual JSON/CSV imports remain recovery-only.
- A final roster is legal with the eight required starters (1 QB, 2 RB, 2 WR, 1 TE, 1 K, 1 DST) and zero to six backups. The August 31 authenticated report contains all 12 legal team rosters and 144 rostered players; every roster is between 8 and 14 players. Roster-dependent advice now gates on that rule, not on 14 players per team. DST nickname/team-code aliases are reconciled at the governed identity boundary, and actual CBS column positions are anchored from the stable trailing salary/contract columns.
...
The separate projection application must fill the exact 717-row template in `artifacts/thunder-bowl/projection-handoff-2026/`. Raw projection files are not an Admin-page import: the browser accepts only a fully validated pack. Run:
...
- Jonnu Smith (`TE-GB`) is now the 717th permanent identity through the strict supplemental catalog: stable ID `fbg:SmitJo02`, bye 11, searchable/assignable/public-sticker identity, and a conservative $1 valuation. The release exception permits only exact manifest identities and rejects unregistered additions, removals, mutations, keeper injection, or changes to the other 716 player records.
- Final QA passed 440/440 tests, the production build, valuation/formula audit, 36-player/608-path advice Monte Carlo, 168-sale full auction, foolproof roster simulation, and 24-keeper/144-sale catastrophe/recovery rehearsal. A deployment-only missing-file failure was caught on the protected live endpoint before promotion; the catalog is now self-contained in the server bundle, and a permanent test forbids supplemental runtime-file dependencies.
- Commits `d85e7e4` and `bad3a87` deployed the supplemental identity and server-bundle hardening. The protected production promotion pinned pack ID `tb26-final-supplemental-catalog-20260829132049` and SHA-256 `6214947472bdd20b16c27e62263525165a4c2a426b099bc09c9b8776ea9d4db5`; a fresh authenticated read returned `status: production`, 717 players, and Jonnu Smith as `TE`, `GB`, bye 11, Market $1. The official ledger remained generation 3 with one `KEEPERS_FINALIZED` event. Remaining owner/device actions are fresh all-player intelligence capture and the MacBook recovery download.
...
- The player pool retains all 717 IDs for search, keyboard order, tiers, and calculations while rendering only the visible window plus overscan. The fixed-height spacer system preserves native scrolling and bounds the initial pool DOM to roughly 13–20 player rows.
...
- The real 717-player pack remains function-bundled and authenticated. The public fallback is a distinct 12-player illustrative pack. The service worker never handles `/api/` requests, so private API responses cannot enter its shared shell cache.
...
- Current-week Footballguys projection evidence is downloaded automatically in the Tuesday job from the official component-stat CSV and rescored with Thunder Bowl rules. The CBS helper captures the signed-in component-stat tables only when the owner presses **Update everything**; the integration does not claim an unsupported unattended CBS authenticated API.
...
- A dual-UTC scheduled function guards on 06:00 America/Denver Tuesday, so daylight and standard time use one local policy. It downloads a fresh Footballguys component-stat projection file before generating the first write-once plan; later live-watch refreshes never overwrite that archive.
