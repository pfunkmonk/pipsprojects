...
- Current authority at this checkpoint: governed final candidate promoted into the repository; 716 players; 12 teams. The exact-byte production lock is the remaining deployment operation described below.
...
- The final operational sequence is immutable: deploy the repository pack, authenticate the private command center, promote that exact pack ID through `/api/thunder-bowl/pack/promote`, verify the served pack reports `production` and the exact ID/hash, then capture fresh all-player intelligence and download the MacBook recovery bundle. Never write a release flag before the deployed bytes match.
