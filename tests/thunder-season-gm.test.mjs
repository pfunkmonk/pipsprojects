...
test("private season shell supports full and per-source updates without auction navigation or caching", async () => {
  ...
  assert.match(source, /await queuePlanRebuild\(\)/);
  ...
  assert.match(worker, /supplemental-session-capture\.mjs\?v=20260922e/);
  assert.match(worker, /season-evidence\.mjs\?v=20260914a/);
  assert.match(worker, /cbs-roster-snapshot\.mjs\?v=20260922e/);
  assert.match(seasonStore, /storage\.setJSON\("plans\/v1\/latest", plan\)/);
  assert.doesNotMatch(seasonStore, /sources\/\$\{plan\.sourceFingerprint\}/);
  assert.doesNotMatch(seasonStore, /\$\{prefix\}\/latest/);
