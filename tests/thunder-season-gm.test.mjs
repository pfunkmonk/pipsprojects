...
test("private season shell supports full and per-source updates without auction navigation or caching", async () => {
  ...
  assert.match(html, /Paste verified FantasyPros capture JSON/);
  assert.match(source, /CBS pasted-data import failed validation/);
  assert.match(source, /FantasyPros pasted-data import failed validation/);
  assert.match(source, /action: "sync-cbs", snapshot/);
  assert.match(source, /action: "capture-fantasypros", capture/);
  ...
  assert.match(source, /savedPlan = \(await postAction\(\{ action: "rebuild-plan" \}\)\)\?\.plan/);
  assert.match(source, /HTTP 504\|server stopped before/);
  assert.match(source, /await waitForQueuedPlan\(previousFingerprint, previousGeneratedAt, source\)/);
  assert.doesNotMatch(source, /void watchQueuedPlan\(previousFingerprint, source\)/);
  ...
  assert.match(worker, /thunder-bowl-season-v62/);
  assert.doesNotMatch(worker, /auctioneer|draft-board|sample-draft-pack/);
  assert.match(worker, /season\.css\?v=20260912b/);
  assert.match(worker, /season\.mjs\?v=20260922d/);
  assert.match(worker, /season-kickoff\.mjs\?v=20260910a/);
  assert.match(worker, /season-news\.mjs\?v=20260901b/);
  assert.match(worker, /fbg-session-capture\.mjs\?v=20260922d/);
  assert.match(worker, /supplemental-session-capture\.mjs\?v=20260922d/);
  assert.match(worker, /season-evidence\.mjs\?v=20260914a/);
  assert.match(worker, /cbs-roster-snapshot\.mjs\?v=20260922d/);
  ...
  assert.match(rootWorker, /thunder-bowl-shell-v142/);
