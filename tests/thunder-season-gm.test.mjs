...
test("private season shell supports full and per-source updates without auction navigation or caching", async () => {
  ...
  assert.match(source, /const result = await postAction\(\{ action: "rebuild-plan" \}\)/);
  assert.match(source, /let savedPlan = result\?\.plan/);
  ...
  assert.match(worker, /thunder-bowl-season-v61/);
  assert.doesNotMatch(worker, /auctioneer|draft-board|sample-draft-pack/);
  assert.match(worker, /season\.css\?v=20260912b/);
  assert.match(worker, /season\.mjs\?v=20260922c/);
  assert.match(worker, /season-kickoff\.mjs\?v=20260910a/);
  assert.match(worker, /season-news\.mjs\?v=20260901b/);
  assert.match(worker, /fbg-session-capture\.mjs\?v=20260922c/);
  assert.match(worker, /supplemental-session-capture\.mjs\?v=20260922c/);
  assert.match(worker, /season-evidence\.mjs\?v=20260914a/);
  assert.match(worker, /cbs-roster-snapshot\.mjs\?v=20260922c/);
