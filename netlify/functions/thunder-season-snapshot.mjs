...
import { getCurrentSeasonSnapshot } from "./_lib/season-service.mjs";
...
export default async function handler(request) {
  try {
    ...
    const plan = await getCurrentSeasonSnapshot({ week: requestedWeek, teamId: requestedTeamId });
    ...
    return json({ error: error?.code === "INVALID_INPUT" ? diagnostic : "The in-season plan could not be loaded safely. The browser may still have the last-known-good private snapshot." }, error?.code === "INVALID_INPUT" ? 400 : 503);
  }
}
