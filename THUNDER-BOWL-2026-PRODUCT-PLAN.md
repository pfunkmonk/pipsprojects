...
- The draft-speed review is implemented: collapsed Live Bid HUD, adjacent Win chance, budget runway, one-key full evidence, surplus heat, three-play nomination assistant, private asset lines, Pro mode, keyboard controls, intel age, equal-height viewport columns with internal secondary scrolling, and an exact-byte one-click final-pack promotion gate. Position-run pressure failed its authority threshold and is therefore a visible warning only.
