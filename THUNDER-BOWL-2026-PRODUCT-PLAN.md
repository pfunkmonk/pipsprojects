...
- The private In-Season GM now has one normal update path: the Chrome Data Helper reads CBS's signed-in all-team roster and current-week component-stat projection tables on demand, while the server downloads Footballguys component-stat projections and refreshes injury, depth, news, and IR evidence in the same action. Provider credentials, cookies, and browser storage remain outside the app and server.
- Authenticated CBS rosters are ready when all 12 teams carry the eight required starters and no more than six backups. The system supports 8–14 players per team; it blocks roster-dependent advice only for a missing required position, fewer than eight players, more than 14 players, or an incomplete 12-team capture.
- The v0.4.0 helper was verified against the live August 31 CBS report: 12 legal teams, 144 rostered players, and 573 governed-catalog players not rostered. A legal roster has the eight required starters and zero to six backups; 14 is a maximum, not a completion target. Live CBS projection coverage is 530 rows (QB 100, RB 100, WR 100, TE 100, K 98, DST 32). Column alignment, defense nickname aliases, and JAC/JAX normalization pass the same source-validation boundary used by the server.
...
2. Maintain a separate live plan during the week. Public injury, status, depth, and news evidence refresh automatically; Footballguys raw weekly projections refresh in the Tuesday job and on **Update everything**. Authenticated CBS league state and CBS raw weekly projections remain user-triggered because they use the owner's signed-in browser session.
