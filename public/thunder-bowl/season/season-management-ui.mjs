...
export function renderManagement(plan, options = {}) {
  ...
  table(outcomes, ["Provider", "Observed player-weeks", "Mean absolute error", "Evidence"], m.outcomes.providers.map((p) => [p.source, p.sampleCount, fmt(p.meanAbsoluteError), p.calibrationReady ? "Descriptive error only—not a calibrated win probability" : "Small sample; do not calibrate from this"]));
